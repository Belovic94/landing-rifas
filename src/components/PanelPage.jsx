...
function fmtDate(s) { if (!s) return "-"; const d = new Date(s); return d.toLocaleString("es-AR"); }
...
function TicketChip({ number, active }) {
  const base =
    ...
    "px-2 py-1 text-xs sm:px-2.5 sm:py-1"; // mobile más compacto
...
export function PanelPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!r.ok) throw new Error("No se pudo cargar /admin/orders");
  ...
  // auto-refresh suave para tiempos (opcional)
  useEffect(() => {
    const i = setInterval(() => setItems((p) => [...p]), 60000);
  ...
  const avgTicketsPerPaidOrder = paidOrders > 0 ? soldTickets / paidOrders : 0;
                    ...
                    >
                      ...
                      >
                        <div class="flex flex-col gap-2">
                          ...
                          {/* Row 3: tickets chips full width */}
                          <div class="w-full">
                            {ticketsCount > 0 ? (
                          ...
                          <div class="mt-2 rounded-xl border border-fame-black/10 bg-fame-soft/10 p-3">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                              ...
                              <div class="text-fame-black/70">
                                ...
                                <span class="font-mono text-xs text-fame-black/80 break-all">{o.paymentId || "-"}</span>
                              </div>
